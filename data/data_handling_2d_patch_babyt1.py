...
def create_npy_data(train_imgs_path, is_extract_more_csf, is_train):
    ...
    for img_dir_name in images_train_dir:
        ...
        # volume         
        img_name  = img_dir_name + '-n3-fli-resize.nii.gz'
        img_name = os.path.join(train_imgs_path, img_dir_name, img_name)
        
        # groundtruth
        img_seg_name  = img_dir_name + '-TissueA-final.nii.gz'
        ...
        for slice in range(img_gtruth_data.shape[2]):
            ...
            if np.count_nonzero(img_gtruth_data[:,:,slice]) and np.count_nonzero(img_data[:,:,slice]):
                ...
                # extract patches of the jth volum train
    ...
    if is_train:
        np.save('imdbs_2d_patch_baby/patches_training_imgs_2d.npy', patches_training_imgs_2d)
        np.save('imdbs_2d_patch_baby/patches_training_gtruth_2d.npy', patches_training_gtruth_2d)

    else:
       np.save('imdbs_2d_patch_baby/patches_val_imgs_2d.npy', patches_training_imgs_2d)
       np.save('imdbs_2d_patch_baby/patches_val_gtruth_2d.npy', patches_training_gtruth_2d)
    print('Saving to .npy files done.')

# extract 2d patches
def extract_2d_patches(img_data, gt_data, is_extract_more_csf):
    ...
    rows = []; cols = []
    for i in range(0,gt_patches.shape[0]):        
        for j in range(0,gt_patches.shape[1]):
            a1 = gt_patches.item((i,j,int(patch_size/2-1),int(patch_size/2-1)))
            a2 = gt_patches.item((i,j,int(patch_size/2-1),int(patch_size/2)))
            a3 = gt_patches.item((i,j,int(patch_size/2),int(patch_size/2-1)))
            a4 = gt_patches.item((i,j,int(patch_size/2),int(patch_size/2)))          
            Sum = a1 + a2 + a3 + a4
            if Sum > 0:
                rows.append(i)
                cols.append(j)
    ...
    if is_extract_more_csf:
        ...
        for i in range(0,gt_patches_csf.shape[0]):        
            for j in range(0,gt_patches_csf.shape[1]):
                ...
                Sum = (a1==10 or a2==10 or a3==10 or a4==10)
                if Sum:
                    rows.append(i)
                    cols.append(j)
...
# load train npy    
def load_train_data():
    imgs_train = np.load('imdbs_2d_patch_baby/patches_training_imgs_2d.npy')
    imgs_gtruth_train = np.load('imdbs_2d_patch_baby/patches_training_gtruth_2d.npy')
    return imgs_train, imgs_gtruth_train

# load validation npy
def load_validatation_data():
    imgs_validation = np.load('imdbs_2d_patch_baby/patches_val_imgs_2d.npy')
    gtruth_validation = np.load('imdbs_2d_patch_baby/patches_val_gtruth_2d.npy')
    return imgs_validation, gtruth_validation

# main
if __name__ == '__main__':
    if 'imdbs_2d_patch_baby' not in os.listdir(os.curdir):
        os.mkdir('imdbs_2d_patch_baby')
    train_imgs_path= '../datasets/Baby_Training'
    val_imgs_path= '../datasets/Baby_Validation'
    print(train_imgs_path)
    print(val_imgs_path)
    is_extract_more_csf = 1
