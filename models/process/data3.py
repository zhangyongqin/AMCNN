...
class dataProcess(object):
	def __init__(self, out_rows, out_cols, data_path = "../deform/train", label_path = "../deform/label", test_path = "../test", npy_path = "../npydata", img_type = "png"):
        # 数据处理类，初始化
...
# 创建训练数据
	def create_train_data(self):
		...
		for imgname in imgs:
			midname = imgname[imgname.rindex("a")+4:]
			datapath = self.data_path + "/" + midname
			labelpath = self.label_path + "/" + midname
...
# 创建测试数据
	def create_test_data(self):
		...
		print('-'*30)
		# fengelabel_path = os.listdir(label_path)
		# 将列表按照数字顺序排列
		# fengelabel_path = sorted(fengelabel_path, key=lambda i: int(re.match(r'(\d+)', i).group()))
		imgs = os.listdir(self.test_path)
		imgs = sorted(imgs, key=lambda i: int(re.match(r'(\d+)', i).group()))
		print(len(imgs))
		imgdatas = np.ndarray((len(imgs),self.out_rows,self.out_cols,1), dtype=np.uint8)
		for imgname in imgs:
			print(imgname)
